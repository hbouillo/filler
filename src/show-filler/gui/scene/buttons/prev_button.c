...
/*   Updated: 2018/03/01 06:40:26 by hbouillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	show = NULL;
	if (!(main->prev_button = sg_create_button(main->ptr)))
		error(ERR_MALLOC, ERR_CRITICAL);
	sg_set_component_boundaries(main->prev_button,
		sg_recti(2 * MAIN_SCENE_MARGIN +
			MAIN_SCENE_FRAME_BUTTON_WIDTH,
			MAIN_SCENE_MARGIN, MAIN_SCENE_FRAME_BUTTON_WIDTH,
			MAIN_SCENE_BUTTON_HEIGHT));
	sg_set_flags(main->prev_button, SG_LOCK_BOTTOM |
		SG_LOCK_LEFT | SG_LOCK_SIZE | SG_ALIGN_TOP | SG_ALIGN_RIGHT);
	sg_set_button_text(main->prev_button, sg_new_gstr("Prev",
