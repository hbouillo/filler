...
/*   Updated: 2018/02/14 05:28:47 by hbouillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				labels(t_show *show, t_main_scene *main)
{
	...
	label.gstr = hgui_new_gstr("Player 1", get_resource_path(font), 35);
	...
	hgui_set_flags(main->ptr, main->p1_label, STATE_COMPONENT_LOCK_TOP
		...
		| HGUI_ALIGN_RIGHT);
	label.gstr = hgui_new_gstr("Player 2", get_resource_path(font), 35);
	bounds = hgui_rectf(0.9, 0.85, 0.0, 0.0);
	...
	hgui_set_flags(main->ptr, main->p2_label, STATE_COMPONENT_LOCK_TOP
		...
		| HGUI_ALIGN_LEFT);
	label.gstr = hgui_new_gstr("vs", get_resource_path(font), 35);
