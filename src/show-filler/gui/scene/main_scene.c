...
/*   Updated: 2018/02/23 18:21:20 by hbouillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				display(t_show *show, t_main_scene *main)
{
	...
	show_set_display_xcolor(main->display_frame, sg_colori(255, 63, 0, 255));
	show_set_display_ocolor(main->display_frame, sg_colori(178, 0, 255, 255));
	show_set_display_ecolor(main->display_frame, sg_colorf(0.3, 0.3, 0.3, 1.0));
	show_set_display_gcolor(main->display_frame, sg_colorf(FILLER_COLOR_BACKGROUND));
...
static void				frames(t_show *show, t_main_scene *main)
{
	...
	// sg_set_component_boundaries(main->main_frame,
	// 	sg_recti(MAIN_SCENE_MARGIN,
	// 			MAIN_SCENE_MARGIN + MAIN_SCENE_BOTTOM_HEIGHT / 2,
	// 			show->win_w - 2 * MAIN_SCENE_MARGIN,
	// 			show->win_h - MAIN_SCENE_MARGIN * 2 -
	// 						MAIN_SCENE_BOTTOM_HEIGHT / 2 -
	// 						MAIN_SCENE_TOP_HEIGHT / 2));
	...
	// sg_set_rectangle_radius(main->main_frame, MAIN_SCENE_CORNER_RADIUS);
	// sg_set_rectangle_edge(main->main_frame, MAIN_SCENE_EDGE);
	...
	sg_set_flags(main->bottom_frame, SG_ALIGN_RIGHT | SG_ALIGN_TOP | SG_LOCK_BOTTOM | SG_LOCK_LEFT | SG_LOCK_RIGHT
		| SG_LOCK_HEIGHT);
	sg_set_rectangle_icolor(main->bottom_frame, sg_colorf(FILLER_COLOR_BACKGROUND));
	sg_set_rectangle_ocolor(main->bottom_frame, sg_colorf(FILLER_COLOR_TEXT));
...
static void				buttons(t_show *show, t_main_scene *main)
{
	...
	sg_set_button_icolor(main->pause_button, sg_colorf(FILLER_COLOR_BACKGROUND));
	sg_set_button_ocolor(main->pause_button, sg_colorf(FILLER_COLOR_TEXT));
	sg_set_button_hcolor(main->pause_button, sg_colorf(FILLER_COLOR_BACKGROUND));
	sg_set_button_pcolor(main->pause_button, sg_colorf(FILLER_COLOR_BACKGROUND));
	sg_set_button_text_color(main->pause_button, sg_colorf(FILLER_COLOR_TEXT));
